...
2. **Early stop**: after each reviewer returns, check its `MAX:` line (see item 4). If it reports `HIGH` or `CRITICAL`, or `BLOCKING: yes`, spawn no further reviewers — go straight to consolidation with the findings collected so far, and note in the executive summary which reviewers were skipped and why.
